...
//password for accesing the library
...
void editbook(void)
{
    ...
    l2:
        ...
        {
        case 1:
            printf("\n\n\t enter the new boook name :");
            scanf("%[^\n]s", be.name);
            break;
        case 2:
            printf("\n\n\t Enter the nwe author name : ");
            scanf("%[^\n]s", be.author);
            break;
        case 3:
...
void listuser(void)
{
    ...
    system("cls");
    printf("\n\n\t\t User Record");
    printf("\n 55s %20s %6s %11s %11s","Id","Name","BookId","Issue Date","Due Date");
    while(!feof(pu))
    {
        if(fread(&u,sizeof(struct user),1,pu)==1)
        printf("\n 55d %20s %6d %2d-%2d-%4d %2d-%2d-%4d",u.id,u.name,u.bookid,u.issue.dd,u.issue.mm,u.issue.yy,u.due.dd,u.due.mm,u.due.yy);
    }
    fclose(pu);
...
{
    if((a.dd>=1)&&(a.dd<=30)&&(a.mm>=1)&&(a.mm<=12)&&(a.yy>2021))
    return 1;
    else
    return 0;
}
            ...
            if(bidcheck(idb)==1)
            {
                if(bc.copiesleft==0)
                {
                    ...
                    do
                    {
                        ...
                        scanf("%d-%d-%d",&u.issue.dd,&u.issue.mm,&u.issue.yy);
...
void collectbook(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(f>0)
            {
                printf("\n\t The due date is %d-%d-%d",u.due.dd,uc.due.mm,uc.due.yy);
...
void searchbook(void)
{
    ...
    do
    {
        ...
        if(i==0)
        printf("\n\t No such book found");
        printf("\n Do you want to search for any other boo ? (Y/N)");
        scanf("%c",&ch);
    }while(ch=='Y'||ch=='y');
    fclose(pb);
    return;
    ...
    do
    {
        ...
        while(!feof(pu))
        {
            ...
            if(u.id==id)
            {
                i++;
                printf("\n\t Search Successful");
                printf("\n\t UserId : %d \n\t Name : %s \n\t Book Id : %d \n\t Issue date : %d-%d-%d \n\t Due Dte : %d=%d-%d",u.id,u.name,u.bookid,u.issue.dd,u.issue.mm,u.issue.yy,u.due.dd,u.due.mm,u.due.yy);
            }
        }
        if(i==0)
        printf("\n\t No such user found");
        printf("\n Do you want to search any other user ? (Y/N)");
        scanf("%c",&ch);
    } while (ch=='Y'||ch=='y');
    fclose(pu);
    return;
